...
/**
 * 道具「整柜打包」整局可用次数。
 * 整柜打包是「一次性特权」：整局只允许打包一次，
 * 配合固定件数 + 扣步，彻底杜绝反复清盘刷分。
 */
export const BUNDLE_USES_PER_GAME = 1;
...
export const POWER_UPS = {
  greenChannel: {
    ...
    description:
      '整局仅此一次的特权：点选棋盘任意格子作为中心，SA 把其周围 3×3 的全部单品打包卖给你。整柜打包一律只算 2 件（按 $1,980 / 件入账，与实际清空格数无关），并消耗 1 步。用在最卡手的那一手，为连消铺路。',
