...
1. **綠色通道 (Green Channel)** — 點擊棋盤任意格，把 3×3 範圍「整櫃打包」：**固定只算 2 件**（與實際清空幾格無關），並**消耗 1 步**（用步數換一次清盤機會，避免空刷）。
2. **限量配貨 (Allocation)** — 隨機將一種普通 Logo 升級為「限量版」；限量版不加價，但按專櫃「買一配一」規則，成交時計 2 件。限量版方塊帶**三重高亮**：金環描邊 + 卡片底光 + 斜向流光（含呼吸脈動），遠距離也能一眼認出。
