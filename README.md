...
開局 15 步，用光就結帳走人。但**只有超額完成的匹配才獎勵步數**，
三消不給、獎勵每步封頂 **+3 步**，因此獎勵只是減緩消耗，永遠不會無限續命：

| 觸發條件 | 獎勵 |
| --- | --- |
| 一次消除形成 4 連 | +1 步 |
| 一次消除形成 5 連及以上 | +2 步 |
| 連消（同一步內第 2 段起，每段） | +1 步 |
| 單步上限 | +3 步 |

十字交叉的組合段不算 5 連（只按 3 連計），避免「湊十字刷步數」。
獎勵觸發時頂欄步數旁會浮出 `+N 步` 提示並伴隨上行鈴音。
...
1. **綠色通道 (Green Channel)** — **整局僅此一次**的特權：點擊棋盤任意格，把 3×3 範圍「整櫃打包」，**固定只算 2 件**（與實際清空幾格無關），並**消耗 1 步**。次數用完卡片即變灰，重開一局才恢復。
2. **限量配貨 (Allocation)** — 隨機將一種普通 Logo 升級為「限量版」；限量版不加價，但按專櫃「買一配一」規則，成交時計 2 件。限量版方塊動用**遊戲引擎全部光效**（見下方「限量版強化高亮」），遠距離也能一眼認出。
...
└── src/
    ...
    ├── game/
    │   ├── Match3Scene.ts     # Phaser 3 場景：棋盤渲染 / Tween 動畫 / 粒子 / 光效 / 手勢（Canvas/WebGL）
    │   └── textures.ts        # 12 張獨立 tile 貼圖加載 + 金幣 / 鈔票 / 限量版光效紋理生成
    ├── components/
...
- **拖拽跟手**：手指按下時起點格會輕微放大並跟隨手指移動（Drag & Drop 手感），鬆開後回彈；被拖的方塊連同**它的全部光效**一起傾斜（光效與方塊共用同一個 `Container`）；
- **交換失敗回彈**：兩格 shake 抖動（不消耗步數）；
- **粒子系統**：`Phaser.Particles` 金色閃光爆破，0/120 FPS 級流暢；
- **爆破 / 炸彈**：細金邊標記（低調，不與限量配貨的高光搶視線）。

### ✨ 限量版強化高亮（引擎光效全開）

限量配貨命中的方塊不是「描個金邊」了事，而是把引擎能給的光效都疊上去，
全部走 **Canvas 程序化紋理 + ADD 加色混合 + GeometryMask 圓角裁切**，零外部素材：

| 層 | 實現 | 效果 |
| --- | --- | --- |
| 金環 | `Graphics` 三層圓角描邊 | 暗色 / 白色卡面上都有對比，縮到 40px 仍可辨識 |
| 柔光光暈 | 預乘 alpha 的徑向漸變 + 自轉 + 呼吸 | 方塊「自帶打光」，不是死板的靜態描邊 |
| 射燈光柱 | 錐形漸變紋理 + 明暗搖曳 | 像專櫃頂燈打在作品上 |
| 玻璃稜邊 | 高光切面紋理 + 圓角遮罩 | 方塊像裝在玻璃展示盒裡，圖案不被蓋住 |
| 流動光帶 | 硬邊窄光帶 + 圓角遮罩 + 往返掃動 | 鍍金表面的鏡面反光 |
| 星芒粒子 | `Phaser.Particles` 四芒星 + EdgeZone | 沿方塊邊圈持續迸出十字星 |
| 金字招牌 | 每幀重繪的 `Graphics` | 4 顆金色光點繞方塊邊框流動 |

**為什麼不會糊掉圖案**：舊版用「大矩形描邊 + 一大塊方形光暈」，光效會溢出到相鄰格、
看起來像一塊糊掉的白色方塊。新版的每一層都只出現在**卡面圓角矩形之內或其邊緣**，
強度也逐層調低，因此高亮很亮、圖案依然清晰。
...
引擎紋理由品牌徽章 Canvas 程序化生成（`src/game/textures.ts`），與 React 版 BrandMark 視覺一致，無外部圖片、離線可用；金幣 / 美金大鈔同樣是 Canvas 程序化紋理（`registerCoinTexture` / `registerBillTexture`）；限量版的光暈 / 玻璃高光 / 流動光帶 / 星芒 / 射燈同樣是程序化紋理（`registerLimitedTextures`）。
